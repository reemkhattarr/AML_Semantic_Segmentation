...
class ImgLbWrapper(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        out = self.base[i]
        # out può essere (img, label, ...) -> prendo solo i primi due
        if isinstance(out, (list, tuple)):
            return out[0], out[1]
        return out  # se il dataset già ritorna (img, label)
...
def train():
    ...
    dspth = '/content/drive/MyDrive/AML_Semantic_Segmentation/data'
    ...
    # torchrun imposta queste env var automaticamente
    ...
    LST_DIR = "/content/drive/MyDrive/AML_Semantic_Segmentation/STDC-Seg/data/list/loveda"
    ...
    # Augment SOLO per train (se non la vuoi: metti None)
    AUG_TRAIN_TYPE = None     # es. 'flip,color' oppure None
    AUG_TRAIN_PROB = 0.5
    ...
    train_dataset = LoveDA(
        root=ROOT,
        list_path=os.path.join(LST_DIR, "train_urban.lst"),
        ...
        augmentation_type=AUG_TRAIN_TYPE,
        aug_prob=AUG_TRAIN_PROB
    )
    train_dataset = ImgLbWrapper(train_dataset)
    print("Num campioni TRAIN:", len(train_dataset))
    ...
    dl = DataLoader(train_dataset,
                    ...
                    pin_memory = False,
                    drop_last = True)
    print('steps_per_epoch =', len(dl))
    steps_per_epoch = floor( len(train_dataset) / (batch_size_per_gpu * world_size) )
    ...
    val_dataset = LoveDA(
        root=ROOT,
        list_path=os.path.join(LST_DIR, "val_rural.lst"),
        num_classes=NUM_CLASSES,
        multi_scale=False,         # randomscale=False
        flip=False,                # val -> False
        ignore_label=IGNORE_LABEL,
        base_size=BASE_SIZE,
        crop_size=CROP_SIZE,
        scale_factor=SCALE_FACTOR,
        augmentation_type=None,    # niente augment in val
        aug_prob=0.0
    )
    val_dataset = ImgLbWrapper(val_dataset)
    print("Num campioni VAL:", len(val_dataset))


    sampler_val = torch.utils.data.distributed.DistributedSampler(val_dataset)
    
    dlval = DataLoader(val_dataset,
                    batch_size = 2,
                    shuffle = False,
                    sampler = sampler_val,
